...
import model.servicos.persistencia.interfaces.TaxaLocacaoDAO;
import model.servicos.persistencia.implementacaoCSV.DescontoLocacaoCSV;
import ui.FrameLoader;
import ui.dialogs.DialogLocacao;
import ui.dialogs.DialogVistoria;
import ui.dialogs.DialogRelatorioLocacao;
...
public final class ListagemLocacoes extends javax.swing.JPanel implements DataChangeListener {
    ...
    public void updateTable() {
        ...
        if (tableModel.getRowCount() > 0) {
            ...
            buttonAdicionarTaxa.setEnabled(true);
            buttonAdicionarDesconto.setEnabled(true);
        } else {
            buttonVer.setEnabled(false);
            buttonEntregarDevolver.setEnabled(false);
            buttonAdicionarTaxa.setEnabled(false);
            buttonAdicionarDesconto.setEnabled(false);
        }
    }

    public void createLocacaoForm(Locacao locacao) {
        DialogLocacao dialogForm = new DialogLocacao(FrameLoader.getFrameMain(), true, locacao);
    ...
    private void buttonVerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonVerActionPerformed
        ...
        DialogRelatorioLocacao dialog = new DialogRelatorioLocacao(FrameLoader.getFrameMain(), true, locacao);
    ...
    private void buttonEntregarDevolverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonEntregarDevolverActionPerformed
        ...
        if (locacao.getStatus() == StatusLocacao.FINALIZADA) {
            ...
            DialogVistoria dialogForm = new DialogVistoria(FrameLoader.getFrameMain(), true, locacao, new Vistoria());
            dialogForm.subscribeListener(this);
            dialogForm.updateFormData();
            dialogForm.setVisible(true);
        }
